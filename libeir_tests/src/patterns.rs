...
", ParseConfig::default()).unwrap();
    ...
    assert!(vm.call(&fun, &[Term::Atom(Symbol::intern("abc")).into()]).unwrap().as_i64() == Some(1));

    assert!(vm.call(&fun, &[Term::Atom(Symbol::intern("def")).into()]).unwrap().as_i64() == Some(2));

    {
        let res = vm.call(&fun, &[Term::Nil.into()]).err().unwrap();
        assert!(res.0.erl_eq(&Term::Atom(Symbol::intern("error")).into()));
        assert!(res.1.erl_eq(&Term::Atom(Symbol::intern("function_clause")).into()));
    }
...
", ParseConfig::default()).unwrap();
    ...
    {
        let res = vm.call(&fun, &[Term::Atom(Symbol::intern("aaa")).into()]).err().unwrap();
        assert!(res.0.erl_eq(&Term::Atom(Symbol::intern("error")).into()));
