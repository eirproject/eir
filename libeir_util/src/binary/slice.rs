...
impl<I, Tr> BitCarrier for BitSlice<I> where I: BitCarrier<T = Tr>, Tr: BitTransport {
    type T = Tr;
    fn bit_len(&self) -> usize {
        self.bit_length
    }
}
impl<I> BitRead for BitSlice<I> where I: BitRead {
    fn read_word(&self, n: usize) -> Self::T {
        ...
        (p1.checked_shl(self.bit_offset).unwrap_or(Self::T::ZERO))
            | (p2.checked_shr(Self::T::BIT_SIZE as u32 - self.bit_offset)
               .unwrap_or(Self::T::ZERO))
    }
}

impl<I> BitWrite for BitSlice<I> where I: BitWrite {

    fn write_word(&mut self, n: usize, data: Self::T, mask: Self::T) {
        // TODO find a way to avoid branch?
        if mask == Self::T::ZERO { return; }
...
impl<I> BitSlice<I> where I: BitCarrier {
...
impl<I> BitSlice<I> where I: BitRead {
...
impl<I> Ord for BitSlice<I> where I: BitRead {
    fn cmp(&self, other: &Self) -> Ordering {
        self.partial_cmp(other).unwrap()
    }
}

impl<I> Hash for BitSlice<I> where I: BitRead {
