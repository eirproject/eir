    ...
    ($typ:ty, $utyp:ty) => {
        ...
        impl BitWrite for $typ {
            fn write_word(&mut self, n: usize, data: u8, mask: u8) {
                ...
                let offset = 8 * (size - 1 - n);
                *self = Self::from_bits(
                    ((self.to_bits()) & !((mask as $utyp) << offset))
                        | ((data as $utyp) << offset)
                );
            }
        }
...
impl<N, Tr> BitCarrier for &N where N: BitCarrier<T = Tr>, Tr: BitTransport {
    type T = Tr;
    fn bit_len(&self) -> usize {
        N::bit_len(self)
    }
}
impl<N> BitRead for &N where N: BitRead {
    fn read_word(&self, n: usize) -> Self::T {
        N::read_word(self, n)
    }
}

impl<N, Tr> BitCarrier for &mut N where N: BitCarrier<T = Tr>, Tr: BitTransport {
    type T = Tr;
    fn bit_len(&self) -> usize {
        N::bit_len(self)
    }
}
impl<N> BitRead for &mut N where N: BitRead {
    fn read_word(&self, n: usize) -> Self::T {
        N::read_word(self, n)
    }
}
impl<N> BitWrite for &mut N where N: BitWrite {
...
impl<Tr> BitCarrier for Vec<Tr> where Tr: BitTransport {
    type T = Tr;
    fn bit_len(&self) -> usize {
        (self as &[Self::T]).bit_len()
    }
}
impl<Tr> BitRead for Vec<Tr> where Tr: BitTransport {
    fn read_word(&self, n: usize) -> Self::T {
        (self as &[Self::T]).read_word(n)
    }
}
impl<Tr> BitWrite for Vec<Tr> where Tr: BitTransport {
    fn write_word(&mut self, n: usize, data: Self::T, mask: Self::T) {
        (self as &mut [Self::T]).write_word(n, data, mask)
    }
}

impl<Tr> BitCarrier for &[Tr] where Tr: BitTransport {
    type T = Tr;
    fn bit_len(&self) -> usize {
        self.len() * 8
    }
}
impl<Tr> BitRead for &[Tr] where Tr: BitTransport {
    fn read_word(&self, n: usize) -> Self::T {
        self.get(n).cloned().unwrap_or(Self::T::ZERO)
    }
}

impl<Tr> BitCarrier for &mut [Tr] where Tr: BitTransport {
    type T = Tr;
    fn bit_len(&self) -> usize {
        self.len() * 8
    }
}
impl<Tr> BitRead for &mut [Tr] where Tr: BitTransport {
    fn read_word(&self, n: usize) -> Self::T {
        self.get(n).cloned().unwrap_or(Self::T::ZERO)
    }
}
impl<Tr> BitWrite for &mut [Tr] where Tr: BitTransport {
    fn write_word(&mut self, n: usize, data: Self::T, mask: Self::T) {
...
impl<Tr> BitWrite for [Tr] where Tr: BitTransport {
    fn write_word(&mut self, n: usize, data: Self::T, mask: Self::T) {
        self.get_mut(n).map(|v| *v = (*v & !mask) | (data & mask));
    }
}
